...
def get_count(p, n_h, n_m):
    ...
    while not q.empty():
        ...
        for spell in spells:
            ...
            ### this should be there but it breaks the code - TODO
            if i_b_h_p <= 0:  # won
                print("WonPoison", i_m_u_p, i_n_h_p, i_n_m_p, i_b_h_p, i_s_l_p, i_p_l_p, i_r_l_p, i_used_p)
                if i_m_u_p <= best_mana:
                    best_mana = i_m_u_p
                    best_used = i_used_p
                continue
...
get_count(p=[sss.strip() for sss in s], n_h=50, n_m=500)
print()


def get_count2(p, n_h, n_m):
    boss = dict()
    for line in p:
        entry, value = line.split(": ", 1)
        boss[entry] = int(value)
...
# get_count2(p=[sss.strip() for sss in s], n_h=50, n_m=500)
# print()
